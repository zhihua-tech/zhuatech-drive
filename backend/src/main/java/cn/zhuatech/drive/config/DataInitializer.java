...
package cn.zhuatech.drive.config;
import cn.zhuatech.drive.domain.DomainCatalog;
import cn.zhuatech.drive.model.*;
import cn.zhuatech.drive.repository.*;
...
@Configuration public class DataInitializer {
 ...
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "DRIVE-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="DRIVE-DEMO-"+String.format("%03d",sequence);
